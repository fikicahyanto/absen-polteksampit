...
// dd($p1);
    ...
    public function _checkUsers($pin,$datetime)
    {
      $user = Absen::where('user_id',$pin)->where('datetime', $datetime)->get();
      return $user;
    }

    public function index()
    {
      $absensi = UD::orderBy('datetime','DESC')->get();
      ...
      foreach ($fp as $key => $value) {
        ...
        for ($a=1; $a < count($buffer); $a++) {
          ...
          if ($data != "") {
            ...
            if (!count($this->_checkUsers($pin, $datetime)) > 0) {
              if (empty($pin && $datetime)){
                echo "gagal";
              }else{
              $create_user[] = [
        ...
        echo count($create_user) . '<br>';
        echo "bates per mesin<br><br>";
        UD::insert($create);
        Absen::insert($create_user);
      }
